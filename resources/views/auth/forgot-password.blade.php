...
<body class="font-sans antialiased h-full text-slate-900 bg-slate-50 flex items-center justify-center p-4">
    ...
    <div class="w-full max-w-md bg-white p-8 sm:p-10 rounded-2xl shadow-xl border border-slate-100 space-y-6">
        ...
        @if (session('status'))
            <div class="p-4 rounded-xl bg-emerald-50 border border-emerald-250 text-emerald-800 text-sm font-medium flex gap-3">
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div class="text-center pt-2">
                    <a href="{{ route('login') }}" class="text-sm font-semibold text-indigo-650 hover:text-indigo-800 transition-colors">
                        Back to Log In
                    </a>
                </div>
            </div>
