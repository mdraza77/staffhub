    ...
    <div
        ...
        <div class="text-center space-y-3">
            ...
            <p class="text-sm text-slate-500 leading-relaxed">
                Thanks for signing up! Before getting started, verify your email address by clicking on the link we just
                emailed to you. If you didn't receive it, we will resend another.
            </p>
        </div>
        ...
        @if (session('status') == 'verification-link-sent')
            <div
                class="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm font-medium flex gap-3 leading-relaxed">
                <svg class="w-5 h-5 text-emerald-650 shrink-0 mt-0.5" fill="none" stroke="currentColor" stroke-width="2"
                    viewBox="0 0 24 24">
        ...
        <div class="space-y-4">
            ...
            <form method="POST" action="{{ route('logout') }}" class="text-center pt-2">
                @csrf
                <button type="submit"
                    class="text-sm font-semibold text-slate-550 hover:text-slate-800 transition-colors focus:outline-none underline">
                    Log Out
                </button>
            </form>
        </div>
