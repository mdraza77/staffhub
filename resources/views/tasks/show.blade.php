    ...
    <div class="mb-6 flex justify-between items-center">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('tasks.index') }}"
                class="text-gray-600 hover:text-indigo-600 transition-colors flex items-center gap-1 font-medium text-sm">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
                <div>
                    @if ($task->project_name)
                        <span class="bg-indigo-50 border border-indigo-100 text-indigo-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider mb-2 inline-block">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 border-t border-b border-gray-100 py-5">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-bold mt-0.5 {{ $isOverdue ? 'text-red-650' : 'text-gray-700' }}">
                            ...
                            @if ($isOverdue)
                                <span class="bg-red-50 text-red-750 text-[10px] font-bold px-1.5 py-0.5 rounded ml-1 uppercase">Overdue</span>
                            @endif
                        </p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="bg-gray-50 border border-gray-150 p-5 rounded-xl text-sm leading-relaxed text-gray-750 prose prose-sm max-w-none">
                        {!! $task->description !!}
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
                <h3 class="text-base font-bold text-gray-800 flex items-center gap-2">
                    ...
                    <span class="bg-gray-100 text-gray-650 text-xs px-2 py-0.5 rounded-full">{{ $task->comments->count() }}</span>
                </h3>
                ...
                <div class="space-y-4 max-h-[380px] overflow-y-auto pr-1">
                    @forelse($task->comments as $comment)
                        <div class="flex gap-3">
                            <div class="h-9 w-9 bg-indigo-50 border border-indigo-100 rounded-full flex items-center justify-center font-bold text-indigo-700 text-sm flex-shrink-0">
                            ...
                            <div class="flex-1 bg-gray-50 border border-gray-100 p-3.5 rounded-xl text-sm">
                                <div class="flex justify-between items-center mb-1 text-xs">
                                    ...
                                    <span class="text-gray-450">{{ $comment->created_at->diffForHumans() }} &bull; {{ $comment->created_at->format('d M, h:i A') }}</span>
                                </div>
                    ...
                    @empty
                        <div class="text-center py-6 text-gray-450 text-sm italic">
                            <i class="fa-solid fa-mug-hot text-2xl mb-1 text-gray-300 block"></i> No remarks or queries posted. Start the discussion below.
                        </div>
                    @endforelse
                </div>
                ...
                <form action="{{ route('tasks.comments.store', $task->id) }}" method="POST"
                    ...
                    @csrf
                    <div class="h-9 w-9 bg-gray-105 border border-gray-200 rounded-full flex items-center justify-center font-bold text-gray-550 text-sm flex-shrink-0 uppercase">
                        {{ strtoupper(substr(Auth::user()->name ?? 'U', 0, 1)) }}
                    </div>
                    <div class="flex-1">
                        <textarea name="comment" rows="2" placeholder="Write a comment, remark, queries etc..."
                            class="w-full border border-gray-300 rounded-lg px-4 py-2.5 outline-none focus:ring-2 focus:ring-indigo-500 bg-white text-sm" required></textarea>
                        <div class="flex justify-end mt-2">
                            <button type="submit"
                                class="bg-indigo-650 hover:bg-indigo-705 text-white px-4 py-2 rounded-lg font-medium text-xs shadow-sm transition-colors flex items-center gap-1">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div class="space-y-3.5">
                    <div class="flex justify-between items-center text-sm border-b border-gray-50 pb-2.5">
                        ...
                        @if ($task->status === 'completed')
                            <span class="bg-green-100 text-green-700 text-xs font-bold px-3 py-1 rounded-full border border-green-200">Completed</span>
                        @elseif($task->status === 'in_progress')
                            <span class="bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1 rounded-full border border-blue-200">In Progress</span>
                        @elseif($task->status === 'ready_for_test')
                            <span class="bg-purple-100 text-purple-700 text-xs font-bold px-3 py-1 rounded-full border border-purple-200">Ready for Test</span>
                        @elseif($task->status === 'testing')
                            <span class="bg-amber-100 text-amber-700 text-xs font-bold px-3 py-1 rounded-full border border-amber-200">Testing</span>
                        @elseif($task->status === 'closed')
                            <span class="bg-gray-100 text-gray-700 text-xs font-bold px-3 py-1 rounded-full border border-gray-200">Closed</span>
                        @else
                            <span class="bg-yellow-105 text-yellow-750 text-xs font-bold px-3 py-1 rounded-full border border-yellow-150">Open</span>
                        @endif
                    </div>
                    ...
                    <div class="flex justify-between items-center text-sm border-b border-gray-50 pb-2.5">
                        ...
                        @if ($task->priority === 'critical')
                            <span class="bg-red-50 border border-red-150 text-red-750 text-xs font-semibold px-2.5 py-0.5 rounded-full"><i class="fa-solid fa-triangle-exclamation mr-0.5"></i> Critical</span>
                        @elseif($task->priority === 'high')
                            <span class="bg-orange-50 border border-orange-150 text-orange-755 text-xs font-semibold px-2.5 py-0.5 rounded-full">High</span>
                        @elseif($task->priority === 'low')
                            <span class="bg-gray-50 border border-gray-200 text-gray-600 text-xs font-semibold px-2.5 py-0.5 rounded-full">Low</span>
                        @else
                            <span class="bg-indigo-50 border border-indigo-150 text-indigo-700 text-xs font-semibold px-2.5 py-0.5 rounded-full">Medium</span>
                        @endif
                    </div>
                </div>
                ...
                <form action="{{ route('tasks.status.update', $task->id) }}" method="POST"
                    ...
                    <div class="flex gap-2">
                        <select name="status"
                            class="flex-1 border border-gray-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-indigo-500 bg-white text-xs" required>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5">
                <h3 class="text-sm font-bold text-gray-800 uppercase tracking-wider flex items-center justify-between">
                    ...
                    <span class="bg-teal-50 border border-teal-100 text-teal-700 text-xs px-2 py-0.5 rounded-full">{{ $task->documents->count() }}</span>
                </h3>

                <!-- Upload Attachment Form -->
                <form action="{{ route('tasks.documents.store', $task->id) }}" method="POST" enctype="multipart/form-data"
                    class="space-y-3 p-3 bg-gray-50 border border-gray-200 rounded-xl">
                    @csrf
                    <div>
                        <label class="block text-[10px] font-bold text-gray-500 uppercase mb-1">Choose File</label>
                        <input type="file" name="document"
                            class="w-full text-xs text-gray-700 border border-gray-300 rounded-lg bg-white px-2 py-1.5 focus:ring-1 focus:ring-indigo-500" required>
                    </div>
                    <div>
                ...
                <div class="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                    @forelse($task->documents as $doc)
                        <div class="p-3 bg-white border border-gray-100 hover:border-gray-200 rounded-xl text-xs space-y-1.5 shadow-2xs hover:shadow-xs transition-shadow">
                            ...
                            @if($doc->remark)
                                <p class="text-gray-600 italic bg-gray-50 p-1.5 rounded border border-gray-100 select-none">{{ $doc->remark }}</p>
                            @endif
                            <div class="flex justify-between items-center text-[10px] text-gray-400 border-t border-gray-50 pt-1.5">
                    ...
                    <div class="relative border-l-2 border-indigo-100 pl-4 ml-1 space-y-4">
                        @foreach($task->statusHistories as $history)
                            <div class="relative text-xs">
                                <div class="absolute -left-[23px] top-1.5 h-2.5 w-2.5 rounded-full bg-indigo-505 border-2 border-white ring-4 ring-indigo-50"></div>
                                ...
                                <p class="text-gray-700 mt-0.5">
                                    Transitioned from 
                                    <span class="bg-gray-100 px-1 py-0.5 rounded font-mono text-[10px]">{{ $history->old_status }}</span> 
                                    to 
                                    <span class="bg-indigo-50 text-indigo-700 font-bold px-1 py-0.5 rounded font-mono text-[10px]">{{ $history->new_status }}</span>
                                </p>
                            </div>
                        @endforeach
                    </div>
